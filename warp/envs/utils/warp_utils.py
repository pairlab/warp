...
def compute_goal_reward(
    ...
    # start_joint_q: wp.array(dtype=float),
    # start_joint_qd: wp.array(dtype=float),
    # outputs
    ...
    #     + (c_ft * (reward_vec_prev[r_i + 4] - reward_vec[r_i + 4])) + task_bonus,
    # )
...
def compute_ctrl_reward(
    ...
    i, j = wp.tid()
    tid = i * num_acts + j  # HARDCODED: assumes act dim per env as 4
